...
window.renderState = {
  ...
  render(view, dom_id, vars = {}) {
    ...
    this.current_view_variables[dom_id] =
      Object.keys(vars).length === 0 ? null : vars;
  ...
  is_being_rendered(view, dom_id) {
    return (
      this.currently_rendered_views.hasOwnProperty(dom_id) &&
      this.currently_rendered_views[dom_id] === view
    );
  },
  unrender(dom_id) {
    for (const k of Object.keys(renderState.currently_rendered_views).filter(
      (e) => e.startsWith("#overlay")
    )) {
...
window.addEventListener(`click`, async (e) => {
  ...
  if (origin && origin.href && !origin.href.startsWith("blob")) {
    e.preventDefault();
    if (origin.extenal) {
      log(
        "LinkDetour",
...
export const alertTags = (title, body, tags, priority) => {
...
const router = () => {
  let simpleRoutes = {
    ...
    "/manage": () => {
      renderState.render("home/manage", "#main");
    },
  };
  const smartRoutes = {
